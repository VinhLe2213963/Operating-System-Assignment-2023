...
{                                                   // no guarantee all given pages are mapped
    ...
    while (fpit != NULL)
    {
        ...
        enlist_pgn_node(&caller->mram->fifo_fp_list, &mm->pgd[pgn + pgit], fpit->fpn);
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
    ...
    for (pgit = 0; pgit < req_pgnum; pgit++)
    {
        ...
        if (freefp_found < 0)
        {
            printf("Alloc pages: not enough free frame in ram\n");
            int swpfpn;
            ...
            /* update page table */
            /* update victim page entry to SWAPPED */
            pte_set_swap(victim_pte, 0, swpfpn);
            /* update new page entry to FPN */
            fpn = victim_fpn;
        }
        // Put the frame number into frm_list
